...
class MatchGame extends Component {
  ...
  getThumbnailId = id => {
    ...
    if (imgUrl === id) {
      this.setState(prevstate => ({count: prevstate.count + 1, thumbId: id}))
      this.renderImage()
    }
  }
  ...
  renderImage = () => {
    const a = imagesList.length
    const b = Math.random() * a
    const c = Math.floor(b)
    const imgurl = imagesList[c].imageUrl
    const imgid = imagesList[c].id
    this.setState({imgUrl: imgurl, imgId: imgid})
  }

  renderFilterImages = () => {
    const {activeTabId} = this.state
    const filteredImages = imagesList.filter(x => x.category === activeTabId)
    return filteredImages
  }

  renderMatchGame = () => {
    const filteredImages = this.renderFilterImages()
    const {activeTabId, imgUrl, imgId} = this.state
    return (
      <div className="main-container2">
        <img src={imgUrl} alt={`match ${imgId}`} className="imgE" />
    ...
    return (
      <div className="main-score-container">
        <div className="score-container">
          ...
          <p className="score-para">{count}</p>
          ...
          <ul className="ul-nav">
            ...
            <li>
              <div className="nav-containerA">
                <p className="score">
                  Score: <span className="spanE">{count}</span>
                </p>
                <div className="time-container">
                  <img
                    ...
                    className="time"
                  />
                  <p className="time-para">{time} sec</p>
                </div>
              </div>
            </li>
          </ul>
        </nav>
        {time <= 0 ? this.renderScoreBoard() : this.renderMatchGame()}
